...
The proof fixture is `workloops-active-spine-conformance.json`. The focused server test `packages/server/src/portfolio-proof-cycle-001.test.ts` executes the fixture through submit, approval, claim, heartbeat, completion, first-class review-evidence attachment, list readback, archive readback, and audit readback. AIQL-compatible review evidence is represented by `aiql-review-evidence.json` and attached through `POST /api/v1/plans/:planId/review-evidence`.

The review evidence is still synthetic. WorkLoops owns the attachment/readback contract only; callers own AIQL, manual, or other review execution.
